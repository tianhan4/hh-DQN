                ...
                if self.step >= self.learn_start:
                    if (self.step-self.learn_start+1) % self.test_step == 0:
                        ...
                        if self.step > 1800:
                            ...
                            qs[:,0,:] = -100
                            print("under 0:")
                            print(np.argmax(qs[0],0)-3)
                            print("under 1:")
                            print(np.argmax(qs[1],0)-3)
                            print("under 2:")
                            print(np.argmax(qs[2],0)-3)
                            print("bs")
                            print(self.model.bs.eval())
                            self.inject_summary({
...
#               self.env.env.monitor.close()
    ...
    def q_learning_mini_batch(self):
        q_loss, summary_str = self.model.learn(*self.memory.sample_more())
        self.writer.add_summary(summary_str, self.step)
        self.writer.flush()
