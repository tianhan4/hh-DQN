...
class Agent(BaseModel):
    def __init__(self, config, sess):
        ...
        self.memory2 = HReplayMemory(config)
                ...
                if self.step - self.init_step > self.learn_start:
                    if (self.step - self.init_step - self.learn_start+1) % self.test_step == 0:
                        ...
                        if self.step > 1800:
                            ...
                            print(tf.matmul(self.model.random_subgoal, tf.nn.l2_normalize(self.model.state_vector,1),
                                            transpose_b=True
                                            ).eval())
                            #print("subgoal parameter")
                            #print(self.model.qs.eval())
