...
class ReplayMemory:
    def __init__(self, config):
        ...
        if config.cnn_format=="NHWC":
            ...
            self.next_states = np.empty((self.memory_size, config.screen_height, config.screen_width, config.history_length),
                                        dtype=np.float16)
            self.res_start_states = np.empty((self.memory_size, config.screen_height, config.screen_width,
                                           config.history_length),
                                         dtype=np.float16)
            self.res_next_states = np.empty((self.memory_size, config.screen_height, config.screen_width, config.history_length),
                                        dtype=np.float16)
            ...
            self.res_start_states = np.empty((self.memory_size, config.history_length, config.screen_height,
                                           config.screen_width), dtype=np.float16)
            self.res_next_states = np.empty((self.memory_size, config.history_length, config.screen_height,
                                          config.screen_width), dtype=np.float16)
        ...
        self.res_prestates = np.empty((self.batch_size,) + self.res_start_states.shape[1:], dtype=np.float16)
        self.res_poststates = np.empty((self.batch_size, ) + self.res_start_states.shape[1:], dtype=np.float16)


    def add(self, state, res_state, n_state,res_n_state, reward, action, terminal, goal, k):
        self.actions[self.current] = action
        self.rewards[self.current] = reward
        self.terminals[self.current] = terminal
        self.start_states[self.current, ...] = state
        self.next_states[self.current, ...] = n_state
        self.res_start_states[self.current, ...] = res_state
        self.res_next_states[self.current, ...] = res_n_state
    ...
    def sample_more(self):
        ...
        while len(actions) < self.batch_size:
            ...
            self.res_prestates[len(actions), ...] = self.res_start_states[index]
            self.res_poststates[len(actions), ...] = self.res_next_states[index]
        ...
        return self.prestates, self.res_prestates, actions, rewards, self.poststates, self.res_poststates,\
               terminals, goals, ks
